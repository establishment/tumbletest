...
std::string Path::GetAbsolutePath(const std::string& current_path) const {
    ...
    if (current_path[0] == '/' or current_path[0] == '~' or current_path[0] == '.') {
        return current_path;
    } else {
...
bool OS::ValidFile(const Path& file) {
    std::string answer = RunBashCommand("( test -e " + file + " ) && echo \"yes\" || echo \"no\"");
    return answer == "yes\n";
}

std::string OS::ReadFile(const Path& file) {
...
void OS::WriteFile(const Path& file, const std::string& content) {
